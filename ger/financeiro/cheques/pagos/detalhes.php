...
?>					
					<table class="tabela-interno" >
						<tr class="tr-interno">
						<td class="botoes-interno"><a href='<?php echo $configUrl; ?>financeiro/cheques/pagos/baixa/<?php echo $dadosNomeCheque['codCheque'];?>/' title='Deseja estornar ?' ><img src='<?php echo $configUrl; ?>f/i/dinheiro-estorno.gif' alt="icone"></a></td>
						</tr>
					</table>
<?php
				}
?>					
					<div class="botao-consultar"><a title="Consultar cheques" href="<?php echo $configUrl;?>financeiro/cheques/pagos/"><div class="esquerda-consultar"></div><div class="conteudo-consultar">Consultar</div><div class="direita-consultar"></div></a></div>					
					<br class="clear" />
				</div>
...
<?php
				if($dadosNomeCheque['statusCheque'] == 'F'){
?>						
						<p class="frase" style="padding-bottom:20px; padding-top:20px; font-weight:bold; color:#718B8F; font-size:18px;">Cheque pago em <strong style="font-size:18px; color:#000;"><?php echo data($dadosNomeCheque['dataDescontoCheque']);?></strong></p>
<?php
				}
				...
				$sqlInformacoes = "SELECT C.*, CL.*, CP.*, CO.* FROM cheques C inner join contasParcial CP on C.codContaParcial = CP.codContaParcial inner join contas CO on CP.codConta = CO.codConta inner join fornecedores CL on CO.codFornecedor = CL.codFornecedor WHERE C.codCheque = ".$url[7];
				$resultInformacoes = $conn->query($sqlInformacoes);
				$dadosInformacoes = $resultInformacoes->fetch_assoc();		

				$_SESSION['fornecedor'] = $dadosInformacoes['nomeFornecedor'];	
...
?>
						<form action="<?php echo $configUrl;?>financeiro/cheques/pagos/detalhes/<?php echo $url[7];?>/" method="post">	
<?php
...
?>
							...
							<p class="bloco-campo-float"><label>Fornecedor:</label>
							<input class="campo" size="44" type="text" <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> name="fornecedor" value="<?php echo $_SESSION['fornecedor'];?>" /></p>
...
<?php
				if($_SESSION['erro'] == "ok"){
					...
					$_SESSION['bnome-fornecedores-filtro'] = "";
