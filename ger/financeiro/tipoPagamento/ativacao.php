		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[6] != ""){
					...
					$sqlCons = "SELECT nomeTipoPagamento, statusTipoPagamento FROM tipoPagamento WHERE codTipoPagamento = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
				
					echo "<div id='filtro'>";
				
					if($dadosCons['statusTipoPagamento'] == "T"){
					...
					$sql =  "UPDATE tipoPagamento SET statusTipoPagamento = '".$alteraStatus."' WHERE codTipoPagamento = ".$url[6];
					$result = $conn->query($sql);
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeTipoPagamento'];
