	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[6] != ""){
					$sqlCons = "SELECT nomeRecibo, statusRecibo FROM recibos WHERE codRecibo = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
					
					if($dadosCons['statusRecibo'] == "T"){
					...
					$sql =  "UPDATE recibos SET statusRecibo = '".$alteraStatus."' WHERE codRecibo = ".$url[6];
