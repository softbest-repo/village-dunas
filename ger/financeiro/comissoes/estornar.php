	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[5] != ""){
					$sqlCons = "SELECT * FROM comissoesCorretores WHERE codComissaoCorretor = ".$url[6]." LIMIT 0,1";
					...
					$sqlConta = "SELECT * FROM contas WHERE codComissaoCorretor = ".$url[6]."";
					...
					$sqlDeleta = "DELETE FROM contas WHERE codComissaoCorretor = ".$url[6]."";
					$resultDeleta = $conn->query($sqlDeleta);
			
					$sql =  "UPDATE comissoesCorretores SET statusComissaoCorretor = '".$alteraStatus."' WHERE codComissaoCorretor = ".$url[6];
