		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[4] != ""){
					...
					$sqlCons = "SELECT * FROM comissoes WHERE codComissao = ".$url[6].$filtraUsuarioPlanilha." LIMIT 0,1";
					...
					if($dadosCons['codComissao'] == "" || $usuario == "G" || $usuario == "C" || $usuario == "CP"){
						...
						$sql =  "DELETE FROM comissoes WHERE codComissao = ".$url[6];
						$result = $conn->query($sql);
				
						$sql =  "DELETE FROM comissoesCorretores WHERE codComissao = ".$url[6];
						$result = $conn->query($sql);
						
						if($result == 1){
							$_SESSION['nome'] = $dadosCons['nomeComissao'];
