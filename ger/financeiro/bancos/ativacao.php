	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[6] != ""){
					$sqlCons = "SELECT nomeBanco, statusBanco FROM bancos WHERE codBanco = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
					
					if($dadosCons['statusBanco'] == "T"){
					...
					$sql =  "UPDATE bancos SET statusBanco = '".$alteraStatus."' WHERE codBanco = ".$url[6];
					$result = $conn->query($sql);
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeBanco'];
						$_SESSION['ativar'] = "ok";
