	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				$sqlFornecedor = "SELECT * FROM fornecedores WHERE codFornecedor = '".$dadosContas['codFornecedor']."' LIMIT 0,1";
...
?>
			<div id="localizacao-topo">
				<div id="conteudo-localizacao-topo">
					...
					<p class="flexa"></p>
					<p class="nome-lista"><?php echo $dadosFornecedor['nomeFornecedor'] ;?> - <?php echo data($dadosContas['dataCConta']);?></p>
					<br class="clear" />
				</div>
...
?>
				<table class="tabela-interno" >
					<tr class="tr-interno">
					<td class="botoes-interno"><a href='<?php echo $configUrl; ?>financeiro/contas-pagar/baixa/<?php echo $dadosContas['codConta'];?>/' title='Deseja estornar <?php echo $dadosContas['nomeConta'] ?>?' ><img src='<?php echo $configUrl; ?>f/i/dinheiro-estorno.gif' alt="icone"></a></td>
					</tr>
				</table>
<?php
				}
?>
				<div class="botao-consultar" style="float:left;"><a title="Consultar Contas a Pagar" href="<?php echo $configUrl;?>financeiro/contas-pagar/"><div class="esquerda-consultar"></div><div class="conteudo-consultar">Consultar</div><div class="direita-consultar"></div></a></div>					
				<br class="clear"/>
			</div>
...
<?php
				...
				if(isset($_POST['vencimento'])){						
					...
					$sqlAltera = "UPDATE contas SET nomeConta = '".$_POST['nome']."', codUsuario = '".$_POST['usuario']."', codTipoPagamento = '".$dadosPegaTipoPagamento['codTipoPagamento']."', vencimentoConta = '".data($_POST['vencimento'])."', tipoPagamentoConta = '".$_POST['tipo-pagamento']."' WHERE codConta = ".$url[6]."";
					$resultAltera = $conn->query($sqlAltera);					
					
					if($resultAltera == 1){
						$_SESSION['alterar'] = "ok";
						$_SESSION['nome'] = $_POST['vencimento'];
						$erroData = "<p class='erro'>Conta a Pagar alterada com sucesso!</p>";
						...
						while($dadosConta = $resultConta->fetch_assoc()){
							...
							$sqlTipoPagamento = "SELECT nomeTipoPagamento FROM tipoPagamento WHERE codTipoPagamento = ".$dadosConta['codTipoPagamento']." LIMIT 0,1";
							...
							$_SESSION['usuario'] = $dadosConta['codUsuario'];
							$_SESSION['tipoPagamentoP'] = $dadosTipoPagamento['nomeTipoPagamento'];
						...
						$erroData = "<p class='erro'>Problemas ao alterar Conta a Pagar!</p>";							
					}
				}else{
					$sqlConta = "SELECT * FROM contas WHERE codConta = ".$url[6]." LIMIT 0,1";
					$resultConta = $conn->query($sqlConta);
				...
				$sqlConfere = "SELECT count(codConta) total FROM contas WHERE codAgrupadorConta = ".$dadosContas['codAgrupadorConta']."";
...
?>
					<div class="area-erro" style="width:auto; display:table; padding-left:30px; padding-right:30px; text-align:center;">
						<p class="erro">Conta a Pagar <strong>extornada</strong> com sucesso!</p>
					</div>
<?php
				}
?>
					<script type="text/javascript">
						function habilitaCampo(){
							...
							document.getElementById("busca_autocomplete_softbest_tipoPagamentoP_c").disabled = false;
					...
					<br/>
					<form name="contas" action="<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $url[6];?>/" method="post">
						...
						<div id="auto_complete_softbest" style="width:236px; float:left; margin-bottom:15px;">
							...
							<input class="campo" type="text" name="fornecedores" required style="width:220px;" <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> value="<?php echo $_SESSION['fornecedores']; ?>" onKeyUp="auto_complete(this.value, 'fornecedores', event);" onkeydown="if (getKey(event) == 13) return false;" autocomplete="off" id="busca_autocomplete_softbest" /></p>
						...
						<p class="bloco-campo-float"><label>Usuário Despesa: <span class="obrigatorio"> </span></label>
							<select class="campo" id="usuario" name="usuario" <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> style="width:150px; height:32px;">
...
?>					
						...
						<div id="auto_complete_softbest" style="width:176px; float:left; margin-bottom:15px;">
							...
							<input class="campo" type="text" name="tipoPagamentoP" required style="width:160px;" <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> value="<?php echo $_SESSION['tipoPagamentoP']; ?>" onClick="auto_complete(this.value, 'tipoPagamentoP_c', event);" onKeyUp="auto_complete(this.value, 'tipoPagamentoP_c', event);" onBlur="fechaAutoComplete('tipoPagamentoP_c');" onkeydown="if (getKey(event) == 13) return false;" autocomplete="off" id="busca_autocomplete_softbest_tipoPagamentoP_c" /></p>
						...
						<input style="width:147px;" class="campo" type="text" required readonly <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> id="valorReceber" name="valorReceber" value="<?php echo $_SESSION['valorReceber']; ?>"/></p>
						...
						<div class="botao-expansivel"><p class="esquerda-botao"></p><input class="botao" <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> type="submit" name="alterar" id="alterar" title="Alterar Conta a Pagar" value="Alterar" onClick="validaForm();"/><p class="direita-botao"></p></div>						
						<br class="clear"/>
					</form>
<?php
...
<?php
					$sqlConta = "SELECT * FROM contas WHERE areaPagamentoConta = 'P' and codAgrupadorConta = ".$dadosContas['codAgrupadorConta']." ORDER BY statusConta ASC, baixaConta ASC, vencimentoConta ASC, codConta DESC";
					$resultConta = $conn->query($sqlConta);
					while($dadosConta = $resultConta->fetch_assoc()){
...
?>
							...
							<tr class="tr">
								<td class="cinco" style="text-align:center;"><a style="<?php echo $dadosConta['vencimentoConta'] <= date('Y-m-d') && $dadosConta['statusConta'] != 'F' && $dadosConta['baixaConta'] != 'F' ? 'color:#FF0000;' : '';?>" href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta receber <?php echo $dadosConta['nomeConta'] ?>'><?php echo $dadosConta['parcelaConta'] == "E" ? 'Entrada' : $dadosConta['parcelaConta'];?></a></td>
								<td class="vinte" style="text-align:center;"><a style="<?php echo $dadosConta['vencimentoConta'] <= date('Y-m-d') && $dadosConta['statusConta'] != 'F' && $dadosConta['baixaConta'] != 'F' ? 'color:#FF0000;' : '';?>" href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta receber <?php echo $dadosConta['nomeConta'] ?>'><?php echo data($dadosConta['vencimentoConta']);?></a></td>
								<td class="vinte" style="text-align:center;"><a style="<?php echo $dadosConta['vencimentoConta'] <= date('Y-m-d') && $dadosConta['statusConta'] != 'F' && $dadosConta['baixaConta'] != 'F' ? 'color:#FF0000;' : '';?>" href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta receber <?php echo $dadosConta['nomeConta'] ?>'><?php echo $dadosConta['statusConta'] == 'F' && $dadosConta['baixaConta'] == 'T' ? 'Paga' : '';?> <?php echo $dadosConta['statusConta'] == 'T' && $dadosConta['baixaConta'] == 'F' ? 'Baixa' : '';?> <?php echo $dadosConta['statusConta'] == 'T' && $dadosConta['baixaConta'] == 'T' ? 'Em Aberto' : '';?></a></td>
								<td class="vinte" style="text-align:center;"><a style="<?php echo $dadosConta['vencimentoConta'] <= date('Y-m-d') && $dadosConta['statusConta'] != 'F' && $dadosConta['baixaConta'] != 'F' ? 'color:#FF0000;' : '';?>" href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta receber <?php echo $dadosConta['nomeConta'] ?>'>R$ <?php echo number_format($valorReceber, 2, ",", ".");?></a></td>
								<td class="vinte" style="text-align:center;"><a style="<?php echo $dadosConta['vencimentoConta'] <= date('Y-m-d') && $dadosConta['statusConta'] != 'F' && $dadosConta['baixaConta'] != 'F' ? 'color:#FF0000;' : '';?>" href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta receber <?php echo $dadosConta['nomeConta'] ?>'>R$ <?php echo number_format($dadosConta['vParcela'], 2, ",", ".");?></a></td>
<?php
						if($dadosConta['statusConta'] == "T" && $dadosConta['baixaConta'] == "T"){
?>
								<td class="botoes"><a href='<?php echo $configUrl; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'];?>/#pagamento' title='Deseja <?php echo $statusPergunta;?> a mensalidade do cliente <?php echo $dadosConta['nomeConta'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icone-pagamento<?php echo $status;?>.gif" alt="icone" /></a></td>
<?php
						}else{
?>
								<td class="botoes"><img style="margin-left:5px;" src="<?php echo $configUrl;?>f/i/default/corpo-default/icone-pagamento<?php echo $status;?>.gif" alt="icone" /></td>
<?php
						}
?>
...
?>
					<p id="pagamento" style="width:1100px;"></p>
<?php
							...
							if(floatval($somaDados) <= floatval($valorReceber)){
								...
								if($resultInsere == 1){
									...
									if($_POST['tipo-pagamento2'] == "CH" && $_POST['nomeCheque'] != "" && $_POST['paraCheque'] != ""){
										...
										$sqlInsere = "INSERT INTO cheques VALUES(0, ".$dadosContaParcial['codContaParcial'].", ".$dadosBancos['codBancoConta'].", '".$_POST['numeroBancoCheque']."', '".$_POST['nomeCheque']."', '".$_POST['agenciaCheque']."', '".$_POST['contaCheque']."', '".$_POST['numeroCheque']."', '".data($_POST['paraCheque'])."', '', 'P', '', 'T')";
										$resultInsere = $conn->query($sqlInsere);
									}
									...
									$limpaValor = str_replace(",", "", $limpaValor);
									$_SESSION['valor'] = $limpaValor;
					
									$sqlContaParcialAcres = "SELECT SUM(acrescimoContaParcial) registros FROM contasParcial WHERE codConta = ".$url[6]."";
									$resultContaParcialAcres = $conn->query($sqlContaParcialAcres);
									$dadosContaParcialAcres = $resultContaParcialAcres->fetch_assoc();

									$valorReceber = $dadosContaParcialDesc['registros'] + $dadosContaParcial['registros'] - $dadosContaParcialAcres['registros'];
									$valorReceber = $dadosContas['vParcela'] - round($valorReceber, 10);
...
?>
					<form name="contasParcial" style="margin-top:40px;" action="<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $url[6];?>/#pagamento" method="post">
						<p class="titulo" style="font-size:18px; color:#718B8F; font-weight:bold;">Cadastrar Pagamento</p>
						<br/>
						<p class="total-receber" style="color:#718B8F; margin-bottom:10px;"><strong>Total a Pagar:</strong> R$ <?php echo number_format($valorReceber, 2, ",", ".");?></p>
						...
						<input type="hidden" id="total-receber" value="<?php echo str_replace(",", "", number_format($_SESSION['valor'], 2));?>" name="valorReceber"/>
						...
						<input type="text" style="width:150px;" class="campo" value="<?php echo $_SESSION['nomePagar'];?>" name="nome-receber"/></p>
...
?>
						...
						<div class="botao-expansivel" style="margin-top:27px;"><p class="esquerda-botao"></p><input class="botao" type="submit" <?php echo $dadosContas['baixaConta'] == "T" ? "" : "disabled='disabled'";?> name="enviar" title="Alterar Conta a Pagar" value="Realizar Pagamento"/><p class="direita-botao"></p></div>						
					</form>	
					<br class="clear"/>
					<script type="text/javascript">
								...
								<div style="width:800px; margin:0 auto;">
									...
									<div id="conteudo-requisicao" style="width:800px; overflow-y:auto; margin-top:40px;">
										<div id="mostra-dados" style="width:100%;">
											...
											<p style="padding-top:20px; padding-top:40px; font-family:Arial; text-align:center;">Pagamos a <span style="font-size:16px; font-weight:bold;"><?php echo $dadosFornecedor['nomeFornecedor'];?></span> a quantia de <span style="font-size:16px; font-weight:bold;" id="valor-imprime">R$ 200,00</span> em <span style="font-size:16px; font-weight:bold;" id="data-imprime">19/12/2015</span>.</p>
					...
					<div id="mostra-parcial" style="width:1105px; margin-top:30px;">
						<table class="tabela-menus" >
							<tr class="titulo-tabela" border="none">
...
?>
							...
							<tr class="tr">
								...
								<td class="vinte" style="text-align:center;"><?php echo $dadosContaParcial['nomeContaParcial'];?></a></td>
								<td class="dez" style="width:13%; text-align:center;"><?php echo $forma;?></td>
								<td class="dez" style="width:13%; text-align:center;">
...
?>	
								...
								<td class="dez" style="width:13%; text-align:center;">R$ <?php echo number_format($dadosContaParcial['descontoContaParcial'], 2, ",", ".");?></a></td>
								<td class="dez" style="width:5%; text-align:center;">R$ <?php echo number_format($dadosContaParcial['acrescimoContaParcial'], 2, ",", ".");?></a></td>
								<td class="dez" style="width:13%; text-align:center;">R$ <?php echo number_format($dadosContaParcial['valorContaParcial'], 2, ",", ".");?></a></td>
...
?>		 								 
							...
							<form id="forms" action="<?php echo $configUrl;?>financeiro/contas-pagar/alterar/<?php echo $url[6];?>/#pagamento" method="post">	
								<input type="hidden" value="" id="manda-cod" name="cod"/>
								<input type="hidden" value="" id="manda-conta" name="contaPgto"/>
							</form>	
							<form id="forms-excluir" action="<?php echo $configUrl;?>financeiro/contas-pagar/alterar/<?php echo $url[6];?>/#pagamento" method="post">	
						...
						<p class="total-receber" style="color:#718B8F; margin-bottom:5px; margin-right:10px; text-align:right;"><strong>Total Pago:</strong> R$ <?php echo number_format($valorRecebido, 2, ",", ".");?></p>
						<p class="total-receber" style="color:#718B8F; margin-bottom:10px; margin-right:10px; text-align:right;"><strong>Total a Pagar:</strong> R$ <?php echo number_format($valorReceber, 2, ",", ".");?></p>
					</div>
				</div>
			</div>
<?php
