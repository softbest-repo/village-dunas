		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($_SESSION['cadastros'] == "ok"){
					$erroConteudo = "<p class='erro'>Conta Pagar <strong>".$_SESSION['nome']."</strong> cadastrada com sucesso!</p>";
					$_SESSION['cadastros'] = "";
				}else	
				if($_SESSION['alterar'] == "ok"){
					$erroConteudo = "<p class='erro'>Conta Pagar <strong>".$_SESSION['nome']."</strong> alterada com sucesso!</p>";
					$_SESSION['alterar'] = "";
				}else	
				if($_SESSION['ativacaos'] == "ok"){
					$erroConteudo = "<p class='erro'>Conta Pagar <strong>".$_SESSION['nome']."</strong> ".$_SESSION['acao']." com sucesso!</p>";
					$_SESSION['ativacaos'] = "";
				}
				...
				if(isset($_POST['fornecedores-filtro'])){
					$sqlFornecedor = "SELECT codFornecedor, nomeFornecedor FROM fornecedores WHERE nomeFornecedor = '".$_POST['fornecedores-filtro']."' ORDER BY codFornecedor ASC LIMIT 0,1";
					$resultFornecedor = $conn->query($sqlFornecedor);
					$dadosFornecedor = $resultFornecedor->fetch_assoc();
					
					$_SESSION['fornecedores-filtro-mensalidades-2'] = $dadosFornecedor['nomeFornecedor'];
					$_SESSION['fornecedores-filtro'] = $dadosFornecedor['nomeFornecedor'];
					$_SESSION['fornecedores-filtro-mensalidades'] = $dadosFornecedor['codFornecedor'];
				}
				...
				if($_SESSION['fornecedores-filtro-mensalidades'] != ""){
					$filtraFornecedor = " and C.codFornecedor = '".$_SESSION['fornecedores-filtro-mensalidades']."'";
					$filtraFornecedor2 = " and CL.codFornecedor = '".$_SESSION['fornecedores-filtro-mensalidades']."'";
				}		
				...
				if(isset($_POST['loteamentoFiltro'])){
					if($_POST['loteamentoFiltro'] != ""){
						$_SESSION['loteamentoFiltro'] = $_POST['loteamentoFiltro'];
					}else{
						$_SESSION['loteamentoFiltro'] = "";
					}
				}	
				
				if($_SESSION['loteamentoFiltro'] != ""){
					$filtraImovel = " and L.codImovel = ".$_SESSION['loteamentoFiltro']."";
				}
				...
				if($_SESSION['usuarioFiltro'] != ""){
					$filtraUsuario = " and CO.codUsuario = ".$_SESSION['usuarioFiltro']."";
					$filtraUsuario2 = " and C.codUsuario = ".$_SESSION['usuarioFiltro']."";
				}
...
?>
				<div id="filtro">
					<div id="localizacao-filtro">
						...
						<p class="flexa"></p>
						<p class="nome-lista">Contas a Pagar</p>
						<br class="clear" />
					</div>
<?php
	$_SESSION['voltarContasUrl'] = "contas-pagar";
	$_SESSION['voltarContasTitulo'] = "Contas a Pagar";
?>					
					<div class="demoTarget">
						<div id="formulario-filtro">
							<form name="filtro" action="<?php echo $configUrl;?>financeiro/contas-pagar/" method="post" >																										
							
								<div id="auto_complete_softbest" style="width:215px; float:left; margin-bottom:15px;">
									<p class="bloco-campo" style="margin-bottom:0px;"><label>Filtrar Fornecedor: <span class="obrigatorio"> </span></label>
									<input class="campo" type="text" name="fornecedores-filtro" style="width:200px;" value="<?php echo $_SESSION['fornecedores-filtro']; ?>" onClick="auto_complete(this.value, 'fornecedores', event);" onKeyUp="auto_complete(this.value, 'fornecedores', event);" onkeydown="if (getKey(event) == 13) return false;" onBlur="fechaAutoComplete('fornecedores');" autocomplete="off" id="busca_autocomplete_softbest_fornecedores" /></p>
									<br class="clear"/>
									
									<div id="exibe_busca_autocomplete_softbest_fornecedores" class="auto_complete_softbest" style="width:215px; display:none;">

									</div>
								</div>

								<p class="bloco-campo-float"><label>Usuário Despesa: <span class="obrigatorio"> </span></label>
									<select class="campo" id="usuarioFiltro" name="usuarioFiltro" style="width:150px; height:32px;">
...
?>					
								...
								<div id="auto_complete_softbest" style="width:170px; float:left; margin-bottom:15px;">
									...
									<input class="campo" type="text" name="tipoPagamento-filtro-contas" style="width:155px;" value="<?php echo $_SESSION['tipoPagamento-filtro-contas-nome']; ?>" onClick="auto_complete(this.value, 'tipoPagamentoP', event);" onKeyUp="auto_complete(this.value, 'tipoPagamentoP', event);" onkeydown="if (getKey(event) == 13) return false;" onBlur="fechaAutoComplete('tipoPagamentoP');" autocomplete="off" id="busca_autocomplete_softbest_tipoPagamentoP" /></p>
									<br class="clear"/>
									
									<div id="exibe_busca_autocomplete_softbest_tipoPagamentoP" class="auto_complete_softbest" style="display:none;">
								...
								<p><label class="label">Status:</label>							
									<select class="campo" id="default-usage-select4" style="width:100px; margin-right:0px;" name="tipo-filtro-status">
										...
										<option value="T" <?php echo $_SESSION['tipo-filtro-status'] == "T" ? '/SELECTED/' : '';?> >A Pagar</option>
										<option value="F" <?php echo $_SESSION['tipo-filtro-status'] == "F" ? '/SELECTED/' : '';?> >Pago</option>
									</select>
								</p>
								...
								<div class="botao-novo" style="margin-left:0px; margin-top:18px;"><a onClick="abreCadastrar()"><div class="esquerda-novo"></div><div class="conteudo-novo">Nova Conta Pagar</div><div class="direita-novo"></div></a></div>
...
<?php
				if(isset($_POST['fornecedores'])){
					...
					$sqlPegaFornecedor = "SELECT * FROM fornecedores WHERE nomeFornecedor = '".$_POST['fornecedores']."' ORDER BY codFornecedor DESC LIMIT 0,1";
					$resultPegaFornecedor = $conn->query($sqlPegaFornecedor);
					$dadosPegaFornecedor = $resultPegaFornecedor->fetch_assoc();

					$sqlPegaTipoPagamento = "SELECT * FROM tipoPagamento WHERE nomeTipoPagamento = '".$_POST['tipoPagamentoP']."' and tipoPagamento = 'P' ORDER BY codTipoPagamento DESC LIMIT 0,1";
					...
					if($_POST['codDespesaImovel'] != ""){
						$codDespesa = $_POST['codDespesaImovel'];
					}else{
						$codDespesa = 0;
					}
					...
					if($dadosPegaTipoPagamento['codTipoPagamento'] != ""){		
						...
						if($dadosPegaFornecedor['codFornecedor'] != ""){
							...
							if($_POST['valor-troca1'] != ""){
								for($i=1; $i<=$_POST['quanti-ref']; $i++){								
									...
									if($valorRef != ""){
										$insert = "INSERT INTO contas VALUES (0, ".$agrupador.", '".$codDespesa."', '".$_POST['usuario']."', '".$_POST['cod-contrato']."', '".$_POST['cod-comissao']."', '', 'N', 'P', 0, ".$dadosPegaFornecedor['codFornecedor'].", '".$dadosPegaTipoPagamento['codTipoPagamento']."', '', '".$_POST['nome']."', '".date('Y-m-d')."', '".data($_POST['data-troca'.$i])."', '".$_POST['bem-troca'.$i]."', '".$valorRef."', '".$valorTotal."', '".$acrescimo."', '".$desconto."', '".$_POST['tipo-pagamentoB'.$i]."', 'T', 'T')";
										...
										if($_POST['tipo-pagamentor'.$i] == "CH"){
											...
											if($_POST['numeroBancoChequer'.$i] != "" && $_POST['nomeChequer'.$i] != "" && $_POST['agenciaChequer'.$i] != "" && $_POST['contaChequer'.$i] != "" && $_POST['numeroChequer'.$i] != "" && $_POST['paraChequer'.$i] != ""){
												...
												if($resultInsereContaParcial == 1){
													...
													$sqlInsereCheque = "INSERT INTO cheques VALUES(0, ".$dadosContaParcial['codContaParcial'].", ".$dadosBancos['codBancoConta'].", ".$_POST['numeroBancoChequer'.$i].", '".$_POST['nomeChequer'.$i]."', '".$_POST['agenciaChequer'.$i]."', '".$_POST['contaChequer'.$i]."', '".$_POST['numeroChequer'.$i]."', '".data($_POST['paraChequer'.$i])."', '', 'P', '', 'T')";
							...
							for($i=1; $i<=$_POST['parcela']; $i++){	
								...
								$insert = "INSERT INTO contas VALUES (0, ".$agrupador.", '".$codDespesa."', '".$_POST['usuario']."', '".$_POST['cod-contrato']."', '".$_POST['cod-comissao']."', '', 'N', 'P', 0, ".$dadosPegaFornecedor['codFornecedor'].", '".$dadosPegaTipoPagamento['codTipoPagamento']."', '', '".$_POST['nome']."', '".date('Y-m-d')."', '".data($dataInicio)."', '".$i."', '".$VparcelRedonda."', '".$valorTotal."', '".$acrescimo."', '".$desconto."', '".$_POST['tipo-pagamento'.$i]."', 'T', 'T')";
								$result = $conn->query($insert);	
								...
								if($_POST['tipo-pagamento'.$i] == "CH"){
									...
									if($_POST['numeroBancoCheque'.$i] != "" && $_POST['nomeCheque'.$i] != "" && $_POST['agenciaCheque'.$i] != "" && $_POST['contaCheque'.$i] != "" && $_POST['numeroCheque'.$i] != "" && $_POST['paraCheque'.$i] != ""){
										...
										if($resultInsereContaParcial == 1){
											...
											$sqlInsereCheque = "INSERT INTO cheques VALUES(0, ".$dadosContaParcial['codContaParcial'].", ".$dadosBancos['codBancoConta'].", ".$_POST['numeroBancoCheque'.$i].", '".$_POST['nomeCheque'.$i]."', '".$_POST['agenciaCheque'.$i]."', '".$_POST['contaCheque'.$i]."', '".$_POST['numeroCheque'.$i]."', '".data($_POST['paraCheque'.$i])."', '', 'P', '', 'T')";
											$resultInsereCheque = $conn->query($sqlInsereCheque);
							...
							if($result == 1){
								...
								if($_POST['cod-comissao'] != ""){
									$sqlUpdate = "UPDATE comissoesCorretores SET statusComissaoCorretor = 'F' WHERE codComissaoCorretor = ".$_POST['cod-comissao']."";
									$resultUpdate = $conn->query($sqlUpdate);
								}
								
								if($_POST['codDespesaImovel'] != ""){
									$sqlUpdate = "UPDATE despesasImovel SET statusDespesaImovel = 'F' WHERE codDespesaImovel = ".$_POST['codDespesaImovel']."";
									$resultUpdate = $conn->query($sqlUpdate);
								}
														
								echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrlGer."financeiro/contas-pagar/'>";									

								$display = "none";
							}else{
								$erroData = "<p class='erro'>Problemas ao cadastrar Conta a Pagar!</p>";			
								$display = "block";
							}
						}else{
							...
							$_SESSION['nome'] = $_POST['nome'];
							$_SESSION['tipoPagamentoP'] = $_POST['tipoPagamentoP'];
						...
						$erroData = "<p class='erro'>O campo referente a <strong>".$_POST['tipoPagamentoP']."</strong> não foi encontrado, se for um novo, você precisa clicar em + Novo Tipo Pagamento.</p>";
						$_SESSION['cod-contrato'] = $_POST['cod-contrato'];
						$_SESSION['fornecedores'] = $_POST['fornecedores'];
						$_SESSION['usuario'] = $_POST['usuario'];
					...
					if($_POST['codDespesaImovel'] != ""){
						$_SESSION['codDespesaImovel'] = $_POST['codDespesaImovel'];
						$_SESSION['fornecedores'] = $_POST['nomeFornecedor'];
						$_SESSION['usuario'] = "";
						$_SESSION['nome'] = $_POST['nomeDespesaImovel'];
						$_SESSION['tipoPagamentoP'] = $_POST['tipoPagamento'];
						$_SESSION['vencimento'] = "";
						$_SESSION['parcela'] = "";
						$_SESSION['tipo-pagamento'] = "";
						$_SESSION['formaPagamento'] = "";
						$_SESSION['imovel'] = number_format($_POST['valorDespesaImovel'], 2, ",", ".");;
						$_SESSION['total'] = number_format($_POST['valorDespesaImovel'], 2, ",", ".");;
						$_SESSION['valorTotal'] = number_format($_POST['valorDespesaImovel'], 2, ",", ".");
						$_SESSION['valorReceber'] = number_format($_POST['valorDespesaImovel'], 2, ",", ".");
						$_SESSION['entrada'] = "";
						$_SESSION['acrescimo'] = "";
						$_SESSION['desconto'] = "";
						$_SESSION['Vparcela'] = "";
						$_SESSION['saldo'] = number_format($_POST['valorDespesaImovel'], 2, ",", ".");						
						$display = "block";						
					}else
					if($_POST['codComissaoCorretor'] != ""){
						$_SESSION['cod-comissao'] = $_POST['codComissaoCorretor'];
						$_SESSION['fornecedores'] = "";
						$_SESSION['usuario'] = "";
						$_SESSION['nome'] = $_POST['nomeComissao'];;
						$_SESSION['tipoPagamentoP'] = "";
						$_SESSION['juros'] = "";
						$_SESSION['vencimento'] = "";
						$_SESSION['parcela'] = "";
						$_SESSION['tipo-pagamento'] = "";
						$_SESSION['formaPagamento'] = "";
						$_SESSION['imovel'] = number_format($_POST['valorComissao'], 2, ",", ".");
						$_SESSION['valorTotal'] = number_format($_POST['valorComissao'], 2, ",", ".");
						$_SESSION['valorReceber'] = number_format($_POST['valorComissao'], 2, ",", ".");
						$_SESSION['entrada'] = "";
						$_SESSION['desconto'] = "";
						$_SESSION['acrescimo'] = "";
						$_SESSION['Vparcela'] = "";
						$_SESSION['saldo'] = number_format($_POST['valorComissao'], 2, ",", ".");												
						$display = "block";						
					}else{					
						$_SESSION['fornecedores'] = "";
						$_SESSION['usuario'] = "";
...
?>						
					<div id="cadastrar" style="display:<?php echo $display;?>; margin-left:30px; margin-top:30px; margin-bottom:30px;">	
							...
							function calculosConta(){
								...
								if(valorImovel != "" && valorImovel != "0"){
									...
									if(pegaSaldo != "" && parcela != "" && parcela != "0" && Vparcela != "" && vencimento != "" && tipoPagamento != ""){
										...
										$ss.post("<?php echo $configUrl;?>financeiro/contas-pagar/atualiza-parcelas.php", {saldo: pegaSaldo, parcela: parcela, Vparcela: Vparcela, vencimento: vencimento, tipoPagamento: tipoPagamento}, function(data){												
						...
						<br/>
						<form name="contas" action="<?php echo $configUrlGer; ?>financeiro/contas-pagar/" method="post">
							<input type="hidden" id="tipoEnvio" name="tipoEnvio" value="" />
							
							<input type="hidden" name="codDespesaImovel" value="<?php echo $_SESSION['codDespesaImovel'];?>"/>
							<input type="hidden" name="cod-comissao" value="<?php echo $_SESSION['cod-comissao'];?>"/>
															
							<div id="auto_complete_softbest" style="width:216px; float:left; margin-bottom:15px;">
								<p class="bloco-campo" style="margin-bottom:0px;"><label>Fornecedor: <span class="obrigatorio"> * </span></label>
								<input class="campo" type="text" name="fornecedores" required style="width:200px;" value="<?php echo $_SESSION['fornecedores']; ?>" onClick="auto_complete(this.value, 'fornecedores_c', event);" onKeyUp="auto_complete(this.value, 'fornecedores_c', event);" onBlur="fechaAutoComplete('fornecedores_c');" onkeydown="if (getKey(event) == 13) return false;" autocomplete="off" id="busca_autocomplete_softbest_fornecedores_c" /></p>
								
								<div id="exibe_busca_autocomplete_softbest_fornecedores_c" class="auto_complete_softbest" style="width:215px; display:none;">

								</div>
							</div>	
							
							<p class="bloco-campo-float"><label>Usuário Despesa: <span class="obrigatorio"> </span></label>
								<select class="campo" id="usuario" name="usuario" style="width:150px; height:32px;">
									<option value="">Geral</option>
<?php
				$sqlUsuarios = "SELECT nomeUsuario, codUsuario FROM usuarios WHERE statusUsuario = 'T' and codUsuario != 4 ORDER BY nomeUsuario ASC";
				$resultUsuarios = $conn->query($sqlUsuarios);
...
?>					
							...
							<div id="auto_complete_softbest" style="width:176px; float:left; margin-bottom:15px;">
								<p class="bloco-campo" style="margin-bottom:0px;"><label>Tipo Pagamento: <span class="obrigatorio"> * </span></label>
								<input class="campo" type="text" name="tipoPagamentoP" required style="width:160px;" value="<?php echo $_SESSION['tipoPagamentoP']; ?>" onClick="auto_complete(this.value, 'tipoPagamentoP_c', event);" onKeyUp="auto_complete(this.value, 'tipoPagamentoP_c', event);" onBlur="fechaAutoComplete('tipoPagamentoP_c');" onkeydown="if (getKey(event) == 13) return false;" autocomplete="off" id="busca_autocomplete_softbest_tipoPagamentoP_c" /></p>
								
								<div id="exibe_busca_autocomplete_softbest_tipoPagamentoP_c" class="auto_complete_softbest" style="display:none;">

								</div>
							</div>
											
							<p class="bloco-campo-float"><label>Descrição: <span class="obrigatorio"> * </span></label>
							<input style="width:150px;" class="campo" type="text" name="nome" required id="nome" value="<?php echo $_SESSION['nome'];?>" /></p>
							
							<br class="clear"/>

							<p class="bloco-campo-float"><label>Valor da Conta:<span class="obrigatorio"> * </span></label>
							<input style="width:150px;" class="campo" type="text" required id="total-imovel" name="imovel" value="<?php echo $_SESSION['imovel']; ?>" onkeyup="moeda(this); calculosConta();"/></p>

							<p class="bloco-campo-float"><label>Acréscimo:</label>
							<input style="width:150px;" class="campo" type="text" disabled="disabled" id="acrescimo" name="acrescimo" value="<?php echo $_SESSION['acrescimo']; ?>" onkeyup="moeda(this); calculosConta();"/></p>

							<p class="bloco-campo-float"><label>Desconto:</label>
							<input style="width:150px;" class="campo" type="text" id="desconto" disabled="disabled" name="desconto" value="<?php echo $_SESSION['desconto']; ?>" onkeyup="moeda(this); calculosConta(this);"/></p>

							<p class="bloco-campo-float"><label>Total:</label>
							<input style="width:150px;" class="campo" type="text" disabled="disabled" id="valor-total" required readonly name="valor" value="<?php echo $_SESSION['total']; ?>" /></p>
							...
							<p class="bloco-campo"><div class="botao-expansivel"><p class="esquerda-botao"></p><input class="botao" type="submit" name="cadastrar" title="Salvar Conta a Pagar" value="Salvar" onClick="validaForm(1);"/><p class="direita-botao"></p></div></p>						
							<br class="clear"/>
						</form>					
					</div>
				</div>	
						...
						<div id="imprime-requisicao" style="width:800px; padding-top:20px; padding-bottom:20px; margin:0 45px auto;">
							<div style="width:800px; margin:0 auto;">
								...
								<div id="topo-requisicao" style="width:800px; border-bottom:2px solid #000000;">	
									...
									<p class="titulo-requisicao" style="text-align:center; padding-bottom:10px; margin:0; font-family:Arial; font-size:24px; font-weight:bold;">Contas a Pagar</p>
								</div>
								<div id="conteudo-requisicao" style="width:800px; overflow-y:auto; margin-top:30px; overflow-x:hidden;">
									<div id="mostra-dados" style="width:100%;">
										<div id="nomes" style="width:100%; margin-bottom:15px;">
											<p style="width:15.4%; float:left; margin:0; padding:1%; font-size:12px; font-weight:bold; font-family:Arial; text-align:center; border:1px dashed #000;">Fornecedor</p>
											...
											<p style="width:8%; float:left; margin:0; padding:1%; font-size:12px; font-weight:bold; font-family:Arial; text-align:center; border:1px dashed #000; border-left:0px;">A Pagar</p>
											...
											<p style="width:8%; float:left; margin:0; padding:1%; font-size:12px; font-weight:bold; font-family:Arial; text-align:center; border:1px dashed #000; border-left:0px;">Pagar</p>
											<br style="clear:both;"/>
										</div>
<?php			
				$sqlConta = "SELECT CO.*, C.* FROM contas CO inner join fornecedores C on CO.codFornecedor = C.codFornecedor WHERE CO.areaPagamentoConta = 'P' and CO.baixaConta = 'T' and (CO.vencimentoConta < '".date('Y-m-d')."' and CO.statusConta = 'T'".$filtraImovel.$filtraUsuario.$filtraFornecedor.$filtraTipo.$filtraStatus.") or CO.baixaConta = 'T' and CO.codFornecedor != ''".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraDia.$filtraMes.$filtraAno.$filtraStatus." ORDER BY CO.baixaConta ASC, CO.statusConta ASC, CO.vencimentoConta ASC, CO.codConta DESC LIMIT 0,60";
				$resultConta = $conn->query($sqlConta);
...
?>
										<div id="fornecedores" style="width:100%; padding-bottom:10px; margin-bottom:10px; border-bottom:1px dashed #000;">
											<p class="nome-cliente" style="width:17.5%; float:left; margin:0; font-family:Arial; font-size:11px; text-align:center;"><?php echo $dadosConta['nomeFornecedor'];?> <?php echo $dadosConta['sobrenomeFornecedor'];?></p>
...
<?php
				...
				$sqlContaGeral = "SELECT SUM(C.vParcela) totalConta FROM contas C inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T'";
				$resultContaGeral = $conn->query($sqlContaGeral);
				$dadosContaGeral = $resultContaGeral->fetch_assoc();
				
				$sqlContaGeralReceber = "SELECT SUM(C.vParcela) totalConta FROM contas C inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T'";
				$resultContaGeralReceber = $conn->query($sqlContaGeralReceber);
				$dadosContaGeralReceber = $resultContaGeralReceber->fetch_assoc();
					
				$sqlContaParcialDescontoTotal = "SELECT SUM(CP.descontoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P'";
				...
				$sqlContaParcialAcrescimoTotal = "SELECT SUM(CP.acrescimoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta WHERE C.areaPagamentoConta = 'P'";
				...
				$sqlContaParcialDescontoAtivo = "SELECT SUM(CP.descontoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T'";
				...
				$sqlContaParcialAcrescimoAtivo = "SELECT SUM(CP.acrescimoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T'";
				...
				$sqlContaParcialAtivo = "SELECT SUM(CP.valorContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T'";
				...
				$sqlContaGeralRecebido = "SELECT SUM(C.vParcela) totalConta FROM contas C inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'F'";
				$resultContaGeralRecebido = $conn->query($sqlContaGeralRecebido);
				$dadosContaGeralRecebido = $resultContaGeralRecebido->fetch_assoc();
				
				$sqlContaParcialInativo = "SELECT SUM(CP.valorContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'F'";
...
?>
										<div id="total" style="width:100%; margin-top:30px; border-top:2px solid #000;">
											<p class="titulo" style="font-weight:bold; font-size:14px; padding-right:15px; padding-top:15px; text-align:right;">Total de Contas Geradas:</p>
											...
											<p class="titulo" style="font-weight:bold; font-size:14px; padding-right:15px; padding-top:15px; text-align:right;">Contas a Pagar:</p>
											...
											<p class="titulo" style="font-weight:bold; font-size:14px; padding-right:15px; padding-top:15px; text-align:right;">Total Acréscimos:</p>
											<p class="total" style="text-align:right; padding-right:15px; padding-top:5px; font-size:14px;">R$ <?php echo number_format($valorTotalGeralAcrescimo, 2, ",", ".");?></p>
											<p class="titulo" style="font-weight:bold; font-size:14px; padding-right:15px; padding-top:15px; text-align:right;">Contas Pagas:</p>
											...
											<p class="titulo" style="font-weight:bold; font-size:14px; padding-right:15px; padding-top:15px; text-align:right;">Total de Contas a Pagar:</p>
											<p class="total" style="text-align:right; padding-right:15px; padding-top:5px; padding-bottom:15px; font-size:14px;">R$ <?php echo number_format($dadosContaGeral['totalConta'], 2, ",", ".");?></p>
											<p class="titulo" style="font-weight:bold; font-size:14px; padding-right:15px; padding-top:15px; text-align:right;">Total Geral:</p>
											<p class="total" style="text-align:right; padding-right:15px; padding-top:5px; padding-bottom:15px; font-size:14px;">R$ <?php echo number_format($valorGeral, 2, ",", ".");?></p>
										</div>
									</div>
								</div>
							</div>
...
<?php
				...
				$sqlConta = "SELECT count(codConta) registros, codConta FROM contas CO inner join fornecedores C on CO.codFornecedor = C.codFornecedor WHERE CO.areaPagamentoConta = 'P' and (CO.vencimentoConta < '".date('Y-m-d')."' and CO.statusConta = 'T'".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraStatus.") or CO.codFornecedor != ''".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraDia.$filtraMes.$filtraAno.$filtraStatus."";				
				$resultConta = $conn->query($sqlConta);
				$dadosConta = $resultConta->fetch_assoc();
...
<?php
				...
				if($url[5] == 1 || $url[5] == ""){
					$pagina = 1;
					$sqlConta = "SELECT CO.*, C.* FROM contas CO inner join fornecedores C on CO.codFornecedor = C.codFornecedor WHERE CO.areaPagamentoConta = 'P' and (CO.vencimentoConta < '".date('Y-m-d')."' and CO.statusConta = 'T'".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraStatus.") or CO.codFornecedor != ''".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraDia.$filtraMes.$filtraAno.$filtraStatus." ORDER BY CO.baixaConta ASC, CO.statusConta ASC, CO.vencimentoConta ASC, CO.codConta DESC LIMIT 0,50";
				}else{
					$pagina = $url[5];
					$paginaFinal = $pagina * 50;
					$paginaInicial = $paginaFinal - 50;
					$sqlConta = "SELECT CO.*, C.* FROM contas CO inner join fornecedores C on CO.codFornecedor = C.codFornecedor WHERE CO.areaPagamentoConta = 'P' and (CO.vencimentoConta < '".date('Y-m-d')."' and CO.statusConta = 'T'".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraStatus.") or CO.codFornecedor != ''".$filtraFornecedor.$filtraUsuario.$filtraTipo.$filtraDia.$filtraMes.$filtraAno.$filtraStatus." ORDER BY CO.baixaConta ASC, CO.statusConta ASC, CO.vencimentoConta ASC, CO.codConta DESC LIMIT ".$paginaInicial.",50";
				}		
				...
				while($dadosConta = $resultConta->fetch_assoc()){
					...
					$sqlUsuarioCP = "SELECT * FROM usuarios WHERE codUsuario = '".$dadosConta['codUsuario']."' LIMIT 0,1";
					$resultUsuarioCP = $conn->query($sqlUsuarioCP);
					$dadosUsuarioCP = $resultUsuarioCP->fetch_assoc();	

					if($dadosUsuarioCP['codUsuario'] != ""){
						$usuario = $dadosUsuarioCP['nomeUsuario'];
					}else{
						$usuario = "Geral";
					}
					...
					if($dadosConta['statusConta'] == 'T' && $dadosConta['baixaConta'] == 'T' || $dadosConta['statusConta'] == 'F' && $dadosConta['baixaConta'] == 'T'){
						$valorAPagar =  $dadosConta['vParcela'] - $dadosContaParcial['registros'] - $dadosContaParcialDesc['registros'] + $dadosContaAcrescimo['registros'];

						$totalAReceber = $totalAReceber + $valorAPagar;					
						$totalDescontos = $totalDescontos + $dadosContaParcialDesc['registros'];					
						$totalRecebido = $totalRecebido + $dadosContaParcial['registros'];					
						$totalDeContas = $totalDeContas + $dadosConta['vParcela'];						
					}
...
?>
							<tr class="tr">
								<td class="vinte" style="width:15%;"><a style="<?php echo $cor;?> <?php echo $bold;?>" href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'><?php echo $dadosConta['nomeFornecedor'];?> <?php echo $dadosConta['sobrenomeFornecedor'];?></a></td>
								<td class="vinte" style="width:12%; text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'><?php echo $usuario;?></a></td>
								<td class="vinte" style="width:12%; text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'><?php echo $dadosConta['nomeConta'];?></a></td>
								<td class="" style="width:15%; text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'><?php echo $dadosTipoPagamento['nomeTipoPagamento'];?></a></td>
								<td class="dez" style="text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'><?php echo $dadosConta['parcelaConta'];?></a></td>
								<td class="dez" style="text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'><?php echo data($dadosConta['vencimentoConta']);?></a></td>
								<td class="dez" style="text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'>R$ <?php echo number_format($valorAPagar, 2, ",", ".");?></a></td>
								<td class="dez" style="text-align:center;"><a style="<?php echo $cor;?> <?php echo $bold;?>"  href='<?php echo $configUrlGer; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'] ?>/' title='Veja os detalhes da conta pagar <?php echo $dadosConta['nomeConta'] ?>'>R$ <?php echo number_format($dadosConta['vParcela'], 2, ",", ".");?></a></td>
<?php
					if($dadosConta['statusConta'] == "T" && $dadosConta['baixaConta'] == "T"){
?>
								<td class="botoes" style="padding:0px;"><a href='<?php echo $configUrl; ?>financeiro/contas-pagar/alterar/<?php echo $dadosConta['codConta'];?>/#pagamento' title='Deseja <?php echo $statusPergunta;?> o pagamento <?php echo $dadosConta['nomeConta'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icone-pagamento<?php echo $status;?>.gif" alt="icone" /></a></td>
<?php
					}else{
?>
...
<?php
					...
					if($dadosConta['baixaConta'] == "T" && $dadosConta['statusConta'] == "T"){
?>
								<td class="botoes" style="padding:0px;"><a href='<?php echo $configUrl; ?>financeiro/contas-pagar/baixa/<?php echo $dadosConta['codConta'];?>/' title='Deseja <?php echo $statusPergunta;?> no pagamento <?php echo $dadosConta['nomeConta'] ?>?' ><img src="<?php echo $configUrl;?>f/i/icone<?php echo $baixa;?>" alt="icone" /></a></td>
<?php
					}else{
?>
...
?>
							<script>
								 function confirmaExclusao(cod, nome){
									...
									if(confirm("Deseja excluir a conta pagar "+nome+"?")){
										window.location='<?php echo $configUrlGer; ?>financeiro/contas-pagar/excluir/'+cod+'/';
									}
								  }
							...
							<form id="form" action="<?php echo $configUrl;?>financeiro/contas-pagar/" method="post">	
...
<?php
				...
				$valorGeral = $dadosContaGeral['totalConta'] - $dadosContaParcialDescontoTotal['totalConta'] + $dadosContaParcialAcrescimoTotal['totalConta'];
?>					
						<div id="total" style="width:50%; float:left; background-color:#f5f5f5; margin-top:20px;">
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-left:15px; padding-top:15px;">Total de Contas Geradas:</p>
							<p class="total" style="padding-left:15px; padding-top:5px; font-size:16px;">R$ <?php echo number_format($dadosContaGeral['totalConta'], 2, ",", ".");?></p>
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-left:15px; padding-top:15px;">Contas a Pagar Geral:</p>
							...
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-left:15px; padding-top:15px;">Contas Pagas Geral:</p>
							...
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-left:15px; padding-top:15px;">Total Geral:</p>
...
<?php
				...
				$sqlContaGeral = "SELECT SUM(C.vParcela) totalConta FROM contas C inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and (C.vencimentoConta < '".date('Y-m-d')."' and C.statusConta = 'T'".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraStatus2.") or C.baixaConta = 'T' and C.codFornecedor != ''".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraDia.$filtraMes.$filtraAno.$filtraStatus2."";
				$resultContaGeral = $conn->query($sqlContaGeral);
				$dadosContaGeral = $resultContaGeral->fetch_assoc();
				
				$sqlContaGeralReceber = "SELECT SUM(C.vParcela) totalConta FROM contas C inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T' and (C.vencimentoConta < '".date('Y-m-d')."' and C.statusConta = 'T'".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraStatus2.") or C.baixaConta = 'T' and C.statusConta = 'T' and C.codFornecedor != ''".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraDia.$filtraMes.$filtraAno.$filtraStatus2."";
				$resultContaGeralReceber = $conn->query($sqlContaGeralReceber);
				$dadosContaGeralReceber = $resultContaGeralReceber->fetch_assoc();
					
				$sqlContaParcialDescontoTotal = "SELECT SUM(CP.descontoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P'".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraDia.$filtraMes.$filtraAno.$filtraStatus2."";
				$resultContaParcialDescontoTotal = $conn->query($sqlContaParcialDescontoTotal);
				$dadosContaParcialDescontoTotal = $resultContaParcialDescontoTotal->fetch_assoc();

				$valorTotalGeralDesconto = $valorTotalGeralDesconto + $dadosContaParcialDescontoTotal['totalConta'];

				$sqlContaParcialAcrescimoTotal = "SELECT SUM(CP.acrescimoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P'".$filtraFornecedor2.$filtraTipo2.$filtraData12.$filtroData22.$filtraStatus2."";
				$resultContaParcialAcrescimoTotal = $conn->query($sqlContaParcialAcrescimoTotal);
				$dadosContaParcialAcrescimoTotal = $resultContaParcialAcrescimoTotal->fetch_assoc();
				...
				$sqlContaParcialAcrescimoAtivo = "SELECT SUM(CP.acrescimoContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T' and (C.vencimentoConta < '".date('Y-m-d')."' and C.statusConta = 'T'".$filtraExcluir2.$filtraFornecedor2.$filtraTipo2.$filtraStatus2.") or C.baixaConta = 'T' and C.statusConta = 'T' and C.codFornecedor != ''".$filtraFornecedor2.$filtraTipo2.$filtraData12.$filtraData22.$filtraStatus2."";
				$resultContaParcialAcrescimoAtivo = $conn->query($sqlContaParcialAcrescimoAtivo);
				$dadosContaParcialAcrescimoAtivo = $resultContaParcialAcrescimoAtivo->fetch_assoc();
				
				$sqlContaParcialAtivo = "SELECT SUM(CP.valorContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'T' and (C.vencimentoConta < '".date('Y-m-d')."' and C.statusConta = 'T'".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraStatus2.") or C.baixaConta = 'T' and C.statusConta = 'T' and C.codFornecedor != ''".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraDia.$filtraMes.$filtraAno.$filtraStatus2."";
				$resultContaParcialAtivo = $conn->query($sqlContaParcialAtivo);
				$dadosContaParcialAtivo = $resultContaParcialAtivo->fetch_assoc();
					
				$valorTotalGeralReceber = $dadosContaGeralReceber['totalConta'] - $dadosContaParcialAtivo['totalConta'] - $dadosContaParcialDescontoAtivo['totalConta'] - $dadosContaParcialAcrescimoAtivo['totalConta'];
				
				$sqlContaGeralRecebido = "SELECT SUM(C.vParcela) totalConta FROM contas C inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'F'".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraDia.$filtraMes.$filtraAno.$filtraStatus2."";
				$resultContaGeralRecebido = $conn->query($sqlContaGeralRecebido);
				$dadosContaGeralRecebido = $resultContaGeralRecebido->fetch_assoc();
				
				$sqlContaParcialInativo = "SELECT SUM(CP.valorContaParcial) totalConta FROM contasParcial CP inner join contas C on CP.codConta = C.codConta inner join fornecedores CL on C.codFornecedor = CL.codFornecedor WHERE C.areaPagamentoConta = 'P' and C.baixaConta = 'T' and C.statusConta = 'F'".$filtraContrato2.$filtraFornecedor2.$filtraTipo2.$filtraDia.$filtraMes.$filtraAno.$filtraStatus2."";
				$resultContaParcialInativo = $conn->query($sqlContaParcialInativo);
				$dadosContaParcialInativo = $resultContaParcialInativo->fetch_assoc();	
...
?>
						<div id="total" style="background-color:#f5f5f5; margin-top:20px; width:100%;">
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-right:15px; padding-top:15px; text-align:right;">Total de Contas Geradas Filtro:</p>
							...
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-right:15px; padding-top:15px; text-align:right;">Contas a Pagar Filtro:</p>
							...
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-right:15px; padding-top:15px; text-align:right;">Total Acréscimos Filtro:</p>
							...
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-right:15px; padding-top:15px; text-align:right;">Contas Pagas Filtro:</p>
							...
							<p class="titulo" style="font-weight:bold; font-size:16px; padding-right:15px; padding-top:15px; text-align:right;">Total Geral Filtro:</p>
