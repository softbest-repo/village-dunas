<?php
	...
	$sqlAutoComplete = "SELECT * FROM tipoPagamento where tipoPagamento = 'R' and locate('".$value."',nomeTipoPagamento) > 0 group by nomeTipoPagamento order by locate('".$value."',nomeTipoPagamento) limit 10";
	...
	if($cont == 0){
?>
		<p class="item" style="float:none; margin-right:0px; font-weight:bold;"><?php echo $value;?><span class="icone-novo" onMouseOver="mouseOverNovoItem();" onClick="novoItem('<?php echo $value;?>', 'tipoPagamentoR_c');">+ Novo Tipo Pagamento</span></p>		
<?php
	}
?>
		...
		<input type="hidden" value="tipoPagamentoR_c" id="id_autocomplete_softbest"/>
