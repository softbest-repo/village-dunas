		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[5] != ""){
					...
					$sqlContaAgrupador = "SELECT codAgrupadorConta FROM contas WHERE codContrato = ".$url[6]." and areaPagamentoConta = 'P' ORDER BY codConta DESC";
					$resultContaAgrupador = $conn->query($sqlContaAgrupador);
					$dadosContaAgrupador = $resultContaAgrupador->fetch_assoc();
					
					$sqlConta = "SELECT * FROM contas WHERE codAgrupadorConta = '".$dadosContaAgrupador['codAgrupadorConta']."'";
					...
					$sqlDeleta = "DELETE FROM contas WHERE codAgrupadorConta = '".$dadosContaAgrupador['codAgrupadorConta']."'";
					$resultDeleta = $conn->query($sqlDeleta);
			
					$sql =  "UPDATE contratos SET statusCContrato = '".$alteraStatus."' WHERE codContrato = ".$url[6];
