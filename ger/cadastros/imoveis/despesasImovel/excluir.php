		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[5] != ""){
					...
					$sqlCons = "SELECT nomeDespesaImovel FROM despesasImovel WHERE codDespesaImovel = ".$url[8]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();

					$sql =  "DELETE FROM despesasImovel WHERE codDespesaImovel = ".$url[8];
					$result = $conn->query($sql);
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeDespesaImovel'];
						$_SESSION['exclusao'] = "ok";
						echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrl."cadastros/imoveis/alterar/".$url[6]."/#despesas'>";
					}
				}else{
					echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrl."cadastros/imoveis/alterar/".$url[6]."/#despesas'>";
				}
