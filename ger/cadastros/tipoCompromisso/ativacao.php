		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[6] != ""){
					...
					$sqlCons = "SELECT nomeTipoCompromisso, statusTipoCompromisso FROM tipoCompromisso WHERE codTipoCompromisso = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
				
					echo "<div id='filtro'>";
				
					if($dadosCons['statusTipoCompromisso'] == "T"){
					...
					$sql =  "UPDATE tipoCompromisso SET statusTipoCompromisso = '".$alteraStatus."' WHERE codTipoCompromisso = ".$url[6];
					$result = $conn->query($sql);
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeTipoCompromisso'];
