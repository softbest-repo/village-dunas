	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				$sqlNomePagamento = "SELECT codCidade, nomeCidade, statusCidade FROM cidade WHERE codCidade = '".$url[6]."' LIMIT 0,1";
...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>cadastros/cidade/ativacao/<?php echo $dadosNomePagamento['codCidade'] ?>/' title='Deseja <?php echo $statusPergunta ?> a cidade <?php echo $dadosNomePagamento['nomeCidade'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>-branco.gif" alt="icone"></a></td>
					...
					<div class="botao-consultar"><a title="Consultar Cidades" href="<?php echo $configUrl;?>cadastros/cidade/"><div class="esquerda-consultar"></div><div class="conteudo-consultar">Consultar</div><div class="direita-consultar"></div></a></div>					
				</div>
...
<?php
				if(isset($_POST['alterar'])){
					...
					$sql = "UPDATE cidade SET nomeCidade = '".$_POST['nome']."', codEstado = '".$_POST['estado']."' WHERE codCidade = '".$url[6]."'";
					...
					if($result == 1){
						...
						$erroData = "<p class='erro'>Problemas ao alterar o cidade!</p>";
					}
				}else{
					$sql = "SELECT * FROM cidade WHERE codCidade = ".$url[6];
					$result = $conn->query($sql);
					$dadosCidade = $result->fetch_assoc();
					$_SESSION['nome'] = $dadosCidade['nomeCidade'];
					$_SESSION['estado'] = $dadosCidade['codEstado'];
				}
...
?>				
						...
						<form action="<?php echo $configUrlGer; ?>cadastros/cidade/alterar/<?php echo $url[6] ;?>/" method="post">
							...
							<p class="bloco-campo"><label>Estado: <span class="obrigatorio"> * </span></label>
								<select class="campo" id="estado" <?php echo $erroAtiva == "ok" ? "" : "disabled='disabled'";?> name="estado" style="width:290px;" required>
