		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[6] != ""){
					...
					$sqlCons = "SELECT nomeCidade FROM cidade WHERE codCidade = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();

					$sql =  "DELETE FROM cidade WHERE codCidade = ".$url[6];
					$result = $conn->query($sql);
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeCidade'];
						$_SESSION['exclusao'] = "ok";
						echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrl."cadastros/cidade/'>";
					}
