	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if(controleUsuario($conn) == "tem usuario"){
			...
			if(validaAcesso($conn, $area) == "ok"){
				...
				if($url[6] != ""){
					$sqlCons = "SELECT nomeFornecedor, statusFornecedor FROM fornecedores WHERE codFornecedor = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
				
					echo "<div id='filtro'>";
					
					if($dadosCons['statusFornecedor'] == "T"){
					...
					$sql =  "UPDATE fornecedores SET statusFornecedor = '".$alteraStatus."' WHERE codFornecedor = ".$url[6];
