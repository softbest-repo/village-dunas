<?php
	...
	$sqlVerifica = "SELECT DISTINCT nomeProprietario, codProprietario FROM proprietarios where locate('".$value."',nomeProprietario) > 0".$filtraUsuario." order by locate('".$value."',nomeProprietario) LIMIT 0,1";
	$resultAutoComplete = $conn->query($sqlAutoComplete);
	$dadosAutoComplete = $resultAutoComplete->fetch_assoc();
