<?php
	if($_COOKIE['codAprovado'.$cookie] != ""){
		...
		$sqlLoteamento = "SELECT * FROM loteamentos L inner join cidades C on L.codCidade = C.codCidade WHERE L.statusLoteamento = 'T' and L.urlLoteamento = '".$url[3]."' ORDER BY L.codLoteamento ASC LIMIT 0,1";
							...
							<div id="conteudo-interno">
								<div id="conteudo-loteamentos-detalhes">
									...
									<div id="mostra-detalhes">
										<p class="titulo"><?php echo $dadosLoteamento['nomeLoteamento'];?></p>
										<p class="cidade"><?php echo $dadosLoteamento['nomeCidade'];?> / <?php echo $dadosLoteamento['estadoCidade'];?></p>
										<div style="display:flex; flex-wrap:wrap; flex-direction:row; align-content:center; justify-content:center; gap:15px;">
<?php
		if($dadosLoteamento['comissaoLoteamento'] != ""){
?>		
										<p class="comissao"><strong>Comissão</strong><br/><?php echo $dadosLoteamento['comissaoLoteamento'];?></p>
<?php
		}
		
		if($dadosLoteamento['taxa48Loteamento'] != "0.00"){
?>
										<p class="taxa" style="color:#002c23; margin-top:20px; border:1px solid #ccc; padding:10px; border-radius:5px;"><strong style="color:#002c23; font-weight:bold;">Base de cálculo</strong><br/>Até 48x - <?php echo $dadosLoteamento['taxa48Loteamento'];?>%</p>
<?php	
		}
		if($dadosLoteamento['taxa90Loteamento'] != "0.00"){
?>
										<p class="taxa" style="color:#002c23; margin-top:20px; border:1px solid #ccc; padding:10px; border-radius:5px;"><strong style="color:#002c23; font-weight:bold;">Base de cálculo</strong><br/>Até 90x - <?php echo $dadosLoteamento['taxa90Loteamento'];?>%</p>
<?php	
		}
		if($dadosLoteamento['taxaLoteamento'] != "0.00" || $dadosLoteamento['taxa90Loteamento'] != "0.00" || $dadosLoteamento['taxa48Loteamento'] != "0.00"){
?>
										<p class="taxa" style="color:#002c23; margin-top:20px; border:1px solid #ccc; padding:10px; border-radius:5px;"><strong style="color:#002c23; font-weight:bold;">Base de cálculo</strong><br/>Até 120x - <?php echo $dadosLoteamento['taxaLoteamento'];?>%</p>
										<p class="simulacao" style="color:#002c23; margin-top:20px; border:1px solid #ccc; padding:10px; border-radius:5px;"><strong style="color:#002c23; font-weight:bold;">Faça uma Simulação</strong><br/><a style="color:#002c23; text-decoration:underline;" target="_blank" href="https://www.idinheiro.com.br/calculadoras/calculadora-financiamento-price/">Acessar Calculadora</a></p>
<?php	
		}
?>
										</div>
											...
											<div class="owl-estampas">
												<div class="row">
													<div class="large-12 columns">
														<div class="loop owl-carousel imagens owl-loaded owl-drag">										
<?php
		$sqlImagens = "SELECT * FROM loteamentosImagens WHERE codLoteamento = ".$dadosLoteamento['codLoteamento']." ORDER BY capaLoteamentoImagem ASC, codLoteamentoImagem ASC";
		$resultImagens = $conn->query($sqlImagens);
		while($dadosImagens = $resultImagens->fetch_assoc()){
?>														
															<p class="imagem" style="width:100%"><a rel="lightbox[roadtrip]" title="<?php echo $dadosLoteamento['nomeLoteamento'];?>" href="<?php echo $configUrlGer.'f/loteamentos/'.$dadosImagens['codLoteamento'].'-'.$dadosImagens['codLoteamentoImagem'].'-W.webp';?>" style="width:100%; height:242px; display:block; border-radius:10px; background:transparent url('<?php echo $configUrlGer.'f/loteamentos/'.$dadosImagens['codLoteamento'].'-'.$dadosImagens['codLoteamentoImagem'].'-W.webp';?>') center center no-repeat; background-size:cover, 100%;"></a></p>
<?php
		}
?>
														</div>
...
<?php
		$sqlLoteamentos = "SELECT * FROM loteamentosAnexos WHERE codLoteamento = ".$dadosLoteamento['codLoteamento']." ORDER BY codLoteamentoAnexo DESC LIMIT 0,1";
		$resultLoteamentos = $conn->query($sqlLoteamentos);
		$dadosLoteamentos = $resultLoteamentos->fetch_assoc();
		
		if($dadosLoteamentos['codLoteamentoAnexo'] != "" || $dadosLoteamento['codLoteamento'] == 9 || $dadosLoteamento['codLoteamento'] == 25 || $dadosLoteamento['codLoteamento'] == 32 || $dadosLoteamento['codLoteamento'] == 37){
?>										
										<div id="anexos-loteamentos">
											<div id="exibe-titulos">
<?php
			$cont = 0;
			
			$sqlLoteamentoAnexos = "SELECT * FROM loteamentosAnexos WHERE codLoteamento = '".$dadosLoteamentos['codLoteamento']."' GROUP BY categoriaLoteamentoAnexo ORDER BY categoriaLoteamentoAnexo ASC LIMIT 0,7";
			$resultLoteamentoAnexos = $conn->query($sqlLoteamentoAnexos);
			while($dadosLoteamentoAnexos = $resultLoteamentoAnexos->fetch_assoc()){
				
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "M"){
					$nomeTipo = "Matrículas";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "FL"){
					$nomeTipo = "Fotos Lotes";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "FM"){
					$nomeTipo = "Fotos Mídias";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "V"){
					$nomeTipo = "Vídeos Mídias";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "MA"){
					$nomeTipo = "Mapa";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "P"){
					$nomeTipo = "Projeto";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "PL"){
					$nomeTipo = "Plantas";
				}else
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] == "O"){
					$nomeTipo = "Obras";
				}
				...
				if($dadosLoteamentoAnexos['categoriaLoteamentoAnexo'] != "MA"){
?>											
												<div id="bloco-<?php echo $dadosLoteamentoAnexos['categoriaLoteamentoAnexo'];?>" class="bloco-item" style="margin-left:0px; <?php echo $cont == 1 ? 'margin-left:0px;' : '';?>" onClick="abreAnexos('<?php echo $dadosLoteamentoAnexos['categoriaLoteamentoAnexo'];?>');">
													<p class="titulo-anexo <?php echo $dadosLoteamentoAnexos['categoriaLoteamentoAnexo'];?>"><?php echo $nomeTipo;?></p>
												</div>
<?php
				}else{
?>
												<a style="flex:1;" target="_blank" href="<?php echo $configUrlGer.'f/loteamentosAnexo/'.$dadosLoteamentoAnexos['codLoteamento'].'-'.$dadosLoteamentoAnexos['codLoteamentoAnexo'].'-O.'.$dadosLoteamentoAnexos['extLoteamentoAnexo'];?>">
													<div id="bloco-mapa" class="bloco-item" style="<?php echo $cont == 0 ? 'margin-left:0px;' : '';?>">
														<p class="titulo-anexo mapa">Mapa PDF</p>
													</div>			
												</a>
<?php					
				}
			...
			if($dadosLoteamento['driveLoteamento'] != ""){
?>
												<a style="flex:1; display:flex;" target="_blank" href="<?php echo $dadosLoteamento['driveLoteamento'];?>">
													<div id="bloco-mapa" class="bloco-item">
														<p class="titulo-anexo drive">Drive</p>
													</div>			
												</a>
<?php
			}

			if($dadosLoteamento['codLoteamento'] == 9 || $dadosLoteamento['codLoteamento'] == 25 || $dadosLoteamento['codLoteamento'] == 32 || $dadosLoteamento['codLoteamento'] == 37){
?>
												<div id="bloco-mapa" class="bloco-item" style="<?php echo $cont == 0 ? 'margin-left:0px;' : '';?>" onClick="carregaMapa(<?php echo $dadosLoteamento['codLoteamento'];?>);">
													<p class="titulo-anexo mapa">Mapa</p>
												</div>
<?php
			}
?>												
...
<?php
			...
			while($dadosLoteamentoAnexos = $resultLoteamentoAnexos->fetch_assoc()){
				...
				$cont++;
?>
											<div id="mostra-anexos-<?php echo $dadosLoteamentoAnexos['categoriaLoteamentoAnexo'];?>" class="mostra-anexos" style="display:none;">
												<table id="tabela-anexo">
													<tr class="titulos-anexos">
														<th>Nome</th>
														<th>Visualizar</th>
														<th>Baixar</th>
													</tr>
<?php
					$sqlLoteamentoAnexo = "SELECT * FROM loteamentosAnexos WHERE codLoteamento = ".$dadosLoteamentos['codLoteamento']." and categoriaLoteamentoAnexo = '".$dadosLoteamentoAnexos['categoriaLoteamentoAnexo']."' ORDER BY categoriaLoteamentoAnexo ASC";
					$resultLoteamentoAnexo = $conn->query($sqlLoteamentoAnexo);
					while($dadosLoteamentoAnexo = $resultLoteamentoAnexo->fetch_assoc()){
?>												
													<tr class="visualizao-anexos">
														<td class="nome"><?php echo $dadosLoteamentoAnexo['nomeLoteamentoAnexo'];?></td>
<?php
						if(strtolower($dadosLoteamentoAnexo['extLoteamentoAnexo']) == "jpg" || strtolower($dadosLoteamentoAnexo['extLoteamentoAnexo']) == "png" || strtolower($dadosLoteamentoAnexo['extLoteamentoAnexo']) == "jpeg" || strtolower($dadosLoteamentoAnexo['extLoteamentoAnexo']) == "gif"){
?>														
														<td class="visualizar"><a rel="lightbox" href="<?php echo $configUrlGer.'f/loteamentosAnexo/'.$dadosLoteamentoAnexo['codLoteamento'].'-'.$dadosLoteamentoAnexo['codLoteamentoAnexo'].'-O.'.$dadosLoteamentoAnexo['extLoteamentoAnexo'];?>">Abrir</a></td>
<?php
						}else{
?>														
														<td class="visualizar"><a target="_blank" href="<?php echo $configUrlGer.'f/loteamentosAnexo/'.$dadosLoteamentoAnexo['codLoteamento'].'-'.$dadosLoteamentoAnexo['codLoteamentoAnexo'].'-O.'.$dadosLoteamentoAnexo['extLoteamentoAnexo'];?>">Abrir</a></td>
<?php
						}
?>
														<td class="download"><a download="<?php echo $dadosLoteamentoAnexo['nomeLoteamentoAnexo'];?>" href="<?php echo $configUrlGer.'f/loteamentosAnexo/'.$dadosLoteamentoAnexo['codLoteamento'].'-'.$dadosLoteamentoAnexo['codLoteamentoAnexo'].'-O.'.$dadosLoteamentoAnexo['extLoteamentoAnexo'];?>">Download</a></td>
													</tr>
<?php
					}
?>										
...
?>
										<script type="text/javascript">
											function abreAnexos(categoria){
												var $tg = jQuery.noConflict();
												if(document.getElementById("mostra-anexos-"+categoria).style.display=="none"){
													$tg(".mostra-anexos").slideUp(250);
													$tg(".bloco-item-ativo").addClass("bloco-item").removeClass("bloco-item-ativo");
													$tg("#mostra-anexos-"+categoria).slideDown(250);
													$tg("#bloco-"+categoria).removeClass("bloco-item").addClass("bloco-item-ativo");
												}else{
													$tg(".mostra-anexos").slideUp(250);
													$tg(".bloco-item-ativo").addClass("bloco-item").removeClass("bloco-item-ativo");													
												}
...
<?php
				$sqlQuadrasLote = "SELECT * FROM quadras Q inner join lotes L on Q.codQuadra = L.codQuadra inner join loteamentos LO on L.codLoteamento = LO.codLoteamento WHERE L.codLoteamento = ".$dadosLoteamento['codLoteamento']." GROUP BY Q.codQuadra ORDER BY Q.nomeQuadra ASC";
...
?>
													...
													<script>
														...
														$rfs(".selectQuadra").on("select2:select", function (e) {
															...
															$rfs.post("<?php echo $configUrl;?>loteamentos/carrega-lotes.php", {codLoteamento: <?php echo $dadosLoteamento['codLoteamento'];?>, codQuadra: quadraSeleciona}, function(data){
...
?>														
														<p class="bloco-campo"><label>Lote:</label><br/>
															<select class="selectLote form-control campo" id="idSelectLote" name="loteFiltroSite" style="width:130px; display: none;">
																...
																<option value="">Todos</option>	
<?php
					$sqlLotesLote = "SELECT * FROM lotes L inner join loteamentos LO on L.codLoteamento = LO.codLoteamento WHERE L.codLoteamento = ".$dadosLoteamento['codLoteamento']." and L.codQuadra = ".$_SESSION['quadraFiltroSite']." GROUP BY L.codLote ORDER BY L.nomeLote ASC";
...
?>
														<p class="bloco-campo"><label>Lote:</label><br/>
															<select class="selectLote form-control campo" id="idSelectLote" name="loteFiltroSite" style="width:130px; display: none;">
																...
																<option value="">Todos</option>	
<?php
					$sqlLotesLote = "SELECT * FROM lotes L inner join loteamentos LO on L.codLoteamento = LO.codLoteamento WHERE L.codLoteamento = ".$dadosLoteamento['codLoteamento']." GROUP BY L.codLote ORDER BY L.nomeLote ASC";
					$resultLotesLote = $conn->query($sqlLotesLote);
					while($dadosLotesLote = $resultLotesLote->fetch_assoc()){				
?>
																<option value="<?php echo trim($dadosLotesLote['codLote']);?>" <?php echo trim($dadosLotesLote['codLote']) == $_SESSION['loteFiltroSite'] ? '/SELECTED/' : ''; ?> ><?php echo trim($dadosLotesLote['nomeLote']);?></option>	
...
?>
													...
													<p class="bloco-campo"><label>Bairro:</label><br/>
														<select class="selectBairro form-control campo" id="idSelectBairro" name="bairroFiltroSite" style="width:250px; display: none;">
															...
															<option value="">Todos</option>	
<?php
					$sqlBairros = "SELECT * FROM bairros B inner join lotes L on B.codBairro = L.codBairro WHERE B.statusBairro = 'T' and L.codLoteamento = ".$dadosLoteamento['codLoteamento']." GROUP BY B.codBairro ORDER BY B.nomeBairro ASC";
...
<?php
				...
				$sqlDisponivel = "SELECT * FROM lotes L inner join quadras Q on L.codQuadra = Q.codQuadra ".$innerJoin." WHERE L.codLoteamento = ".$dadosLoteamento['codLoteamento']." and L.statusLote = 'T'".$filtraQuadra.$filtraLote.$filtraBairro.$filtraPosicao.$filtraPrecoDe.$filtraPrecoAte.$filtraStatus."";
...
?>
															<div id="topo-requisicao" style="padding:5px 10px; height:65px; border:1px solid #002c23; border-radius:10px;">	
																...
																<div style="float:left; display:contents;">
																	<p style="font-family:Arial; margin:0; font-size:18px; text-align:left; color:#002c23; font-weight:bold;"><?php echo $dadosLoteamento['nomeLoteamento'];?></span></p>
																	<p style="float:left; font-family:Arial; min-width:160px; margin:0; font-size:13px; text-align:left; color:#002c23; padding-top:5px;"><?php echo $dadosLoteamento['nomeCidade'];?> / <?php echo $dadosLoteamento['estadoCidade'];?></p>
...
?>
																	<div id="clientes" style="width:100%; display:flex; flex-direction:row; flex-wrap:nowrap; justify-content:center; <?php echo $background;?>">
																		<p style="width:13%; float:left; margin:0; font-size:11px; padding:1.7% 1%; font-family:Arial; border:1px dashed #000; text-align:center; border-top:none; font-weight:bold;">[ <?php echo $dadosLote['nomeLote'];?> ] - <?php echo $dadosLote['nomeQuadra'];?><span style="font-weight:normal; font-size:11px;"> - <?php echo $frase;?></span></p>
																		<p style="width:20.15%; float:left; margin:0; padding:0.6% 1%; border:1px dashed #000; border-left:none; border-top:none; text-align:center;"><span style="width:inherit; height:32px; display:table-cell; vertical-align:middle; font-size:11px; font-family:Arial;"><?php echo $dadosBairro['nomeBairro'];?></span></p>
																		<p style="width:12%; float:left; margin:0; font-size:11px; padding:1.7% 1%; font-family:Arial; text-align:center; text-align:center; border:1px dashed #000; border-left:none; border-top:none;"><?php echo $dadosLote['frenteLote'];?> <?php echo $dadosLote['frenteLote'] != "" ? 'x' : '';?> <?php echo $dadosLote['fundosLote'];?> <?php echo $dadosLote['frenteLote'] != "" ? '=' : '';?> <?php echo $dadosLote['areaLote'];?>m²</p>
																		<p style="width:12%; float:left; margin:0; font-size:11px; padding:1.7% 1%; font-family:Arial; text-align:center; text-align:center; border:1px dashed #000; border-left:none; border-top:none;"><?php echo $dadosLote['posicaoLote'];?></p>
																		<p style="width:15%; float:left; margin:0; font-size:11px; padding:0.6% 1%; font-family:Arial; text-align:center; text-align:center; border:1px dashed #000; border-left:none; border-top:none;"><span style="width:inherit; height:32px; display:table-cell; letter-spacing:-0.3px; vertical-align:middle; font-size:11px; font-family:Arial;"><?php echo $exibeValores;?></span></p>
																		<p style="width:15%; float:left; margin:0; font-size:11px; padding:0.6% 1%; font-family:Arial; text-align:center; text-align:center; border:1px dashed #000; border-left:none; border-top:none; <?php echo $backgroundColor;?>"><span style="width:inherit; height:32px; display:table-cell; vertical-align:middle; font-size:12px; font-family:Arial; <?php echo $color;?>"><?php echo $acoes;?></span></p>
																		<br style="clear:both;"/>
																	</div>					
<?php
						if($contPagina == 23 && $mostrando < $totalItens){
							$contPagina = 0;
							$contPagina2++;
?>
																</div>
...
<?php
		...
		$sqlLotes = "SELECT * FROM lotes L inner join quadras Q on L.codQuadra = Q.codQuadra ".$innerJoin." WHERE L.codLoteamento = ".$dadosLoteamento['codLoteamento']." and L.statusLote = 'T'".$filtraQuadra.$filtraLote.$filtraBairro.$filtraPosicao.$filtraPrecoDe.$filtraPrecoAte.$filtraStatus." GROUP BY L.codLote ORDER BY L.statusLote ASC".$ordenar.", CAST(Q.nomeQuadra AS UNSIGNED) ASC, Q.nomeQuadra ASC, CAST(L.nomeLote AS UNSIGNED) ASC, L.nomeLote ASC";
		...
		while($dadosLotes = $resultLotes->fetch_assoc()){
			...
			if($_SESSION['statusFiltroSite'] == "D" && $dadosReservas['codLoteReserva'] == "" || $_SESSION['statusFiltroSite'] != "D"){
				...
				if($contCor == 0){
					...
					$background = "background-color:#e0fff8;";
					$contCor = 0;
				}				
?>													
													<tr class="tr-lista" style="<?php echo $background;?>">
														<td style="font-weight:bold;"><?php echo $dadosLotes['nomeLote'];?><br/><span style="font-weight:normal;"><?php echo $frase;?></span></td>
...
<?php
				if($dadosLotes['entradaLote'] == "0.00" && $dadosLotes['entradaELote'] == "0"){
					if($dadosLoteamento['entradaLoteamento'] == "0.00" && $dadosLoteamento['entradaELoteamento'] == "0"){
						...
						if($dadosLoteamento['tipoELoteamento'] == "P"){
							$entrada = ($dadosLotes['precoLote'] * $dadosLoteamento['entradaELoteamento']) / 100;
						}else{
							$entrada = $dadosLoteamento['entradaLoteamento'];
						}
					}
						...
						if($dadosReservas['codCorretor'] != ""){
							if($dadosReservas['codCorretor'] == $_COOKIE['codAprovado'.$cookie]){
								if($dadosLotes['vendidoLote'] == "F"){
									...
									$exibeValores = "R$ ".number_format($entrada, 2, ",", ".")." +'.$reforcoText.'<br/><strong>".$maiorParcela."x</strong> de <strong>R$ ".$valorParcela."</strong>";									
								}
							}else{
